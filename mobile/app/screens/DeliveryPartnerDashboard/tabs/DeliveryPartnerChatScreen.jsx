...
const DeliveryPartnerChatScreen = ({ navigation }) => {
  ...
  const fetchConversations = async (showLoading = true) => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}/api/chat/delivery-partner/conversations`
      );
      ...
      console.error(
        "Error fetching delivery partner conversations:",
        error.response.data
      );
    } finally {
  ...
  useEffect(() => {
    if (
      socketMessage &&
      socketMessage.type === "REFRESH_DELIVERY_PARTNER_CONVERSATIONS"
    ) {
  ...
  const ConversationItem = ({ conversation }) => (
    ...
    >
      ...
      <View className="mr-3">
        ...
        {conversation.delivery_partner_unread_count > 0 && (
          <View className="absolute flex items-center justify-center w-5 h-5 bg-green-600 rounded-full -top-1 -right-1">
            <Text className="text-xs font-bold text-white">
              {conversation.delivery_partner_unread_count > 9
                ? "9+"
                : conversation.delivery_partner_unread_count}
            </Text>
          </View>
        )}
      </View>
      ...
      <View className="flex-1">
        ...
        <Text
          className={`text-sm ${conversation.delivery_partner_unread_count > 0 ? "text-gray-900 font-medium" : "text-gray-600"}`}
          numberOfLines={2}
        >
    ...
    return (
      <View className="flex-1 bg-gray-50">
        ...
        <View className="items-center justify-center flex-1 px-6">
          ...
          <TouchableOpacity
            className="px-6 py-3 mt-6 bg-green-600 rounded-lg"
      ...
      {conversations.length === 0 ? (
        ...
        >
          <View className="items-center justify-center flex-1 px-6">
            ...
            <Text className="mt-2 text-center text-gray-500">
              Start delivering orders to chat with sellers
            </Text>
            <TouchableOpacity
              className="px-6 py-3 mt-6 bg-green-600 rounded-lg"
              onPress={() => navigation.navigate("Deliveries")}
            >
              <Text className="font-semibold text-white">View Deliveries</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      ) : (
...
export default DeliveryPartnerChatScreen;
