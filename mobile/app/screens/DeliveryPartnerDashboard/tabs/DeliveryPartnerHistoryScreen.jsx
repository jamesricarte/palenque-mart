...
const DeliveryPartnerHistoryScreen = () => {
  ...
  const fetchDeliveryHistory = async () => {
    try {
      const response = await axios.get(
        `${API_URL}/api/delivery-partner/delivery-history`,
        {
          params: {
  ...
  const renderDeliveryCard = (delivery) => (
    <TouchableOpacity
      key={`${delivery.assignment_id}-${delivery.source_type || "assignment"}`}
    ...
    >
      ...
      <View className="pt-3 border-t border-gray-100">
        ...
        {delivery.pickup_time && (
          <Text className="text-xs text-gray-500">
            Picked up: {formatDate(delivery.pickup_time)}
          </Text>
        )}
        {delivery.delivery_time && (
          <Text className="text-xs text-gray-500">
            Delivered: {formatDate(delivery.delivery_time)}
          </Text>
        )}
        {delivery.updated_at && (
          <Text className="text-xs text-gray-500">
