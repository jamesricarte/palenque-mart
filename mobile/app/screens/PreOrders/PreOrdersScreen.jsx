    ...
    >
      <View className="flex-row items-center justify-between mb-3">
        ...
        <View className="px-2 py-1 bg-blue-100 rounded-full">
          <Text className="text-xs font-medium text-blue-600">Pre-Order</Text>
        </View>
      </View>
      ...
      {order.items.map((item, index) => (
        <View key={index} className="flex-row items-center p-2 mb-3 rounded-md bg-gray-50">
          {item.product_image_url ? (
            <Image source={{ uri: item.product_image_url }} className="w-12 h-12 mr-3 rounded-md" resizeMode="cover" />
          ) : (
          ...
          <View className="flex-1">
            ...
            <Text className="text-xs text-gray-500">
              {item.quantity} x ₱{Number.parseFloat(item.unit_price).toFixed(2)} ({formatUnitType(item.unit_type)})
            </Text>
            <View className="flex-row items-center justify-between mt-1">
              <View className={`px-2 py-1 rounded-full ${getPreOrderStatusColor(item.preorder_status)}`}>
                <Text className="text-xs font-medium">{getPreOrderStatusLabel(item.preorder_status)}</Text>
              </View>
            ...
            {item.expected_availability_date && (
              <Text className="text-xs text-blue-600 mt-1">
                Expected: {new Date(item.expected_availability_date).toLocaleDateString()}
              </Text>
            )}
          </View>
        </View>
      ...
      <View className="flex-1 bg-white">
        <View className="flex-row items-center justify-between px-4 pt-16 pb-5 bg-white border-b border-gray-200">
          <View className="flex-row items-center">
            ...
            <Text className="ml-4 text-xl font-semibold">My Pre-Orders</Text>
          </View>
        </View>
        ...
        <View className="items-center justify-center flex-1 px-6">
          ...
          <Text className="mt-2 text-center text-gray-500">Please login to view your pre-orders</Text>
          <TouchableOpacity
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View className="flex-row gap-3">
            {statusOptions.map((option) => (
              <TouchableOpacity
                key={option.key}
                className={`px-4 py-2 rounded-full ${selectedStatus === option.key ? "bg-blue-600" : "bg-gray-200"}`}
      ...
      >
        {orders.length > 0 ? (
          ...
          <View className="items-center justify-center px-6 flex-1 h-[70vh]">
            <Feather name="clock" size={80} color="#9CA3AF" />
            <Text className="mt-4 text-xl font-semibold text-gray-600">No pre-orders found</Text>
