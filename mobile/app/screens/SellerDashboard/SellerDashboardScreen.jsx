...
import CreateLivestreamScreen from "../CreateLivestream/CreateLivestreamScreen"

const Tab = createBottomTabNavigator()

const SellerDashboard = ({ navigation }) => {
  const { enterSellerDashboard, exitSellerDashboard, setTriggerWebSocket, refreshOrdersData, setSellerId } = useSeller()
  ...
  const CustomTabBar = ({ state, descriptors, navigation }) => {
    return (
      <View className="flex-row bg-white border-t border-gray-200" style={{ height: 80 }}>
        {state.routes.map((route, index) => {
          ...
          return (
            <TouchableOpacity key={route.key} onPress={onPress} className="flex-1 items-center justify-center py-2">
              <View className="items-center">
                ...
                <Text className={`text-xs mt-1 ${isFocused ? "text-blue-600" : "text-gray-500"}`}>{label}</Text>
              </View>
            </TouchableOpacity>
          )
        })}
