...
const SellerOverviewScreen = ({ navigation }) => {
  ...
  const fetchTransactions = async (page = 1, status = selectedFilter, reset = false) => {
    try {
      setError(null)
  ...
  const onRefresh = async () => {
    setRefreshing(true)
    await Promise.all([fetchAnalytics(), fetchTransactions(1, selectedFilter, true)])
    setRefreshing(false)
  }
    ...
    <View className="p-4 mb-3 bg-white border border-gray-200 rounded-lg shadow-sm">
      <View className="flex-row items-center justify-between mb-3">
        <View className="flex-1">
          <Text className="text-lg font-semibold text-gray-900">Order #{transaction.order_number}</Text>
          <Text className="text-sm text-gray-600">
            {transaction.customer_first_name} {transaction.customer_last_name}
          </Text>
        </View>
        <View className="items-end">
          <View className={`px-3 py-1 rounded-full ${getStatusColor(transaction.payment_status)}`}>
            <Text className="text-xs font-medium capitalize">{transaction.payment_status}</Text>
          </View>
          <Text className="mt-1 text-lg font-bold text-green-600">{formatCurrency(transaction.seller_amount)}</Text>
        </View>
      </View>
      ...
      <View className="flex-row items-center justify-between py-2 border-t border-gray-100">
        <View className="flex-row items-center">
          ...
          <Text className="ml-1 text-sm text-gray-500">{transaction.items_count} item(s)</Text>
        </View>
        <View className="flex-row items-center">
          <MaterialCommunityIcons name="credit-card-outline" size={16} color="#6b7280" />
          <Text className="ml-1 text-sm text-gray-500 capitalize">{transaction.payment_method.replace(/_/g, " ")}</Text>
        </View>
        <Text className="text-sm text-gray-500">{formatDate(transaction.transaction_date)}</Text>
      </View>
        ...
        <View className="p-4">
          <View className="p-6 mb-6 bg-white border border-gray-200 rounded-lg shadow-sm">
            <View className="flex-row items-center justify-between">
              <View className="flex-1">
                ...
                <Text className="mt-1 text-gray-600">Here's how your store is performing today</Text>
                <View className="flex-row items-center mt-2">
                  <View className="w-2 h-2 mr-2 bg-green-500 rounded-full" />
                  <Text className="text-sm font-medium text-green-600">Store Active</Text>
                </View>
              </View>
          ...
          {loading && transactions.length === 0 ? (
            ...
            <View className="items-center px-5 py-6 mx-4 my-2 bg-white border border-gray-200 rounded-lg shadow-sm">
              ...
              <Text className="mb-2 text-lg font-semibold text-center text-gray-800">Something went wrong</Text>
            ...
            <>
              ...
              {/* Revenue Overview */}
              <View className="mb-4">
                <Text className="mb-3 text-lg font-semibold text-gray-900">Revenue Overview</Text>
                <View className="p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
                  {analytics.revenueOverTime.length > 0 ? (
                    <>
                      <View className="flex-row items-center justify-between mb-4">
                        <View className="flex-row items-center">
                          ...
                          <Text className="ml-2 text-gray-600">Total Revenue</Text>
                        </View>
                      ...
                      <View className="flex-row justify-between">
                        <View className="items-center flex-1">
                          <Text className="text-2xl font-bold text-gray-900">
                            {analytics.revenueOverTime.reduce((sum, item) => sum + item.ordersCount, 0)}
                          </Text>
                          <Text className="text-sm text-gray-600">Total Orders</Text>
                        </View>
                        <View className="items-center flex-1">
                          <Text className="text-2xl font-bold text-gray-900">
                            {analytics.revenueOverTime.reduce((sum, item) => sum + item.itemsSold, 0)}
                          </Text>
                          <Text className="text-sm text-gray-600">Items Sold</Text>
                        </View>
                      </View>
                    </>
                  ) : (
                    <View className="items-center py-8">
                      ...
                      <Text className="mt-2 text-center text-gray-500">No revenue data available</Text>
                    </View>
                  )}
                </View>
              </View>
              ...
              {analytics.paymentMethodBreakdown.length > 0 && (
                <View className="mb-4">
                  <Text className="mb-3 text-lg font-semibold text-gray-900">Payment Methods</Text>
                  <View className="p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
                    {analytics.paymentMethodBreakdown.map((method, index) => (
                      ...
                      >
                        <View className="items-center justify-center w-10 h-10 mr-3 bg-blue-100 rounded-lg">
                          <MaterialCommunityIcons name="credit-card-outline" size={20} color="#3b82f6" />
                        </View>
                        <View className="flex-1">
                          <Text className="font-medium text-gray-900 capitalize">
                            {method.method.replace(/_/g, " ")}
                          </Text>
                          <Text className="text-sm text-gray-600">{method.orderCount} orders</Text>
                        </View>
                        <Text className="text-lg font-bold text-green-600">{formatCurrency(method.totalAmount)}</Text>
                      </View>
                    ))}
                  </View>
                </View>
              ...
              {analytics.orderStatusBreakdown.length > 0 && (
                <View className="mb-4">
                  <Text className="mb-3 text-lg font-semibold text-gray-900">Order Status</Text>
                  <View className="p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
                    {analytics.orderStatusBreakdown.map((status, index) => (
                      ...
                      >
                        <View className="items-center justify-center w-10 h-10 mr-3 bg-orange-100 rounded-lg">
                          <MaterialIcons name="assignment" size={20} color="#ea580c" />
                        </View>
                        <View className="flex-1">
                          <Text className="font-medium text-gray-900 capitalize">
                            {status.status.replace("_", " ")}
                          </Text>
                          <Text className="text-sm text-gray-600">{status.orderCount} orders</Text>
                        </View>
                        {status.totalAmount > 0 && (
                          <Text className="text-lg font-bold text-green-600">{formatCurrency(status.totalAmount)}</Text>
                        )}
                      </View>
                    ))}
                  </View>
              ...
              <View className="mb-4">
                <View className="flex-row items-center justify-between mb-4">
                  ...
                  <View className="flex-row items-center">
                    ...
                    <Text className="ml-1 text-sm text-gray-600">{transactions.length} transactions</Text>
                  </View>
                </View>
                    ...
                    {statusFilters.map((filter) => (
                      <TouchableOpacity
                        ...
                        className={`px-4 py-2 rounded-full border ${
                          selectedFilter === filter.value ? "bg-blue-600 border-blue-600" : "bg-white border-gray-200"
                        }`}
                ...
                {filterLoading ? (
                  <View className="items-center justify-center p-8 bg-white border border-gray-200 rounded-lg shadow-sm">
                    ...
                    <Text className="mt-2 text-sm text-gray-500">Filtering transactions...</Text>
                  </View>
                  ...
                  <>
                    ...
                    {hasMoreTransactions && (
                      ...
                      >
                        ...
                        <Text className="ml-2 font-medium text-blue-600">Load More Transactions</Text>
                      </TouchableOpacity>
                    )}
                  </>
                ) : (
                  <View className="items-center justify-center p-8 bg-white border border-gray-200 rounded-lg shadow-sm">
                    ...
                    <Text className="mt-4 text-lg font-medium text-gray-600">No Transactions Found</Text>
        ...
        <View className="justify-end flex-1" style={{ backgroundColor: "rgba(0, 0, 0, 0.5)" }}>
          <View className="bg-white rounded-t-lg">
            <View className="flex-row items-center justify-between p-4 border-b border-gray-200">
              <Text className="text-lg font-semibold text-gray-900">Select Period</Text>
            ...
            {periods.map((period) => (
              ...
              >
                ...
                {selectedPeriod === period.value && <Ionicons name="checkmark" size={20} color="#2563eb" />}
              </TouchableOpacity>
            ))}
          </View>
        </View>
