...
const reviewSellerDocument = async (req, res) => {
  ...
  try {
    const [documents] = await connection.execute(
      "SELECT * FROM seller_documents WHERE id = ?",
      [documentId]
    );
    ...
    await connection.execute(
      "UPDATE seller_documents SET verification_status = ?, rejection_reason = ? WHERE id = ?",
    ...
    if (action === "reject") {
      ...
      await connection.execute(
        "UPDATE seller_applications SET status = 'needs_resubmission', updated_at = NOW() WHERE id = ?",
        [applicationId]
      );
      ...
      const [appDetails] = await connection.execute(
        "SELECT account_type FROM seller_applications WHERE id = ?",
        [applicationId]
      );

      const requiredDocTypes = ["government_id", "selfie_with_id"];
      if (appDetails[0].account_type === "business") {
      ...
      if (allRequiredDocsVerified) {
        // If all are verified, move the application back to 'under_review' for final approval
        await connection.execute(
...
module.exports = reviewSellerDocument;
