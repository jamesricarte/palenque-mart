...
const resubmitDocuments = async (req, res) => {
  ...
  try {
    ...
    for (const file of files) {
      const documentId = file.fieldname

      // Get doc details and verify ownership and status
      const [documents] = await connection.execute(
        "SELECT * FROM seller_documents WHERE id = ? AND application_id = ?",
        [documentId, application.id],
      )

      if (documents.length === 0) {
        throw new Error(`Document with ID ${documentId} not found for this application.`)
      }
      const document = documents[0]

      if (document.verification_status !== "rejected") {
        throw new Error(`Document ${document.document_type} is not in a rejected state.`)
      }
      ...
      const newFilePath = `user_${userId}/${application.application_id}/${document.document_type}-${Date.now()}${fileExt}`
      const { error: uploadError } = await supabase.storage
        .from(BUCKET_NAME)
        .upload(newFilePath, file.buffer, { contentType: file.mimetype })

      if (uploadError) {
        throw new Error(`Supabase upload failed for ${document.document_type}: ${uploadError.message}`)
      }
    ...
    const hasRemainingRejectedDocs = allDocs.some((doc) => doc.verification_status === "rejected")
    ...
    return res.status(500).json({
      message: error.message || "Something went wrong while resubmitting documents.",
      success: false,
